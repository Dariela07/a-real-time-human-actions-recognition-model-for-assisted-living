...
model = dict(
    ...
    backbone=dict(
        ...
        attention_type='divided_space_time',  # Fixed attention type here
    ...
    cls_head=dict(
        ...
        loss_cls=dict(
            ...
            loss_weight=1.0,
            class_weight=[1.0, 1.0, 1.0, 0.3]  # Adjust class imbalance
        )
    ),
    data_preprocessor=dict(
        ...
        format_shape='NCTHW'  # Fixed formatting
    )
)
...
optim_wrapper = dict(
    optimizer=dict(
        type='SGD', lr=0.0001, momentum=0.9, weight_decay=1e-4, nesterov=True),
    paramwise_cfg=dict(
        custom_keys={
