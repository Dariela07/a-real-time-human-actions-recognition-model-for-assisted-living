...
train_cfg = dict(
    type='EpochBasedTrainLoop', max_epochs=60, val_begin=1, val_interval=2  # Fewer epochs for fine-tuning
)
...
optim_wrapper = dict(
    optimizer=dict(type='SGD', lr=0.0001, momentum=0.9, weight_decay=1e-4),  # Smaller LR for fine-tuning
...
param_scheduler = [
    ...
    dict(
        ...
        eta_min=0,
        by_epoch=True,
        begin=7,   #begin for CosineAnnealingLR to match the end of the LinearLR scheduler. end for CosineAnnealingLR to the total number of epochs
        end=60
    )
]
...
## Hooks
default_hooks = dict(
    checkpoint=dict(interval=3, max_keep_ckpts=5),  # Save checkpoints periodically
    logger=dict(interval=25), # 100
